...
export default function UserCreateForm({ onClose, onSuccess }) {
  ...
  const {
    ...
    defaultValues: {
      username: "",
      password: "",
      confirm_password: "", // snake_case to match serializer
      email: "",
      is_active: false,
      is_superuser: false,
    },
  ...
  const onSubmit = async (data) => {
    try {
      await createUser(data); // send confirm_password too
      onClose?.();
      router.refresh();
  ...
  return (
    ...
    >
      <h1 className="text-2xl font-bold text-center mb-6">Nuevo Usuario</h1>
      ...
      {/* Superuser (optional, only if admin) */}
      <div className="flex items-center gap-3 bg-gray-50 p-3 rounded">
        <input
          type="checkbox"
